...
(function (BrowserAPI) {
    ...
    BrowserAPI.isBrowserAPISupported = isBrowserAPISupported;
    ...
    BrowserAPI.getBrowser = getBrowser;
    function getSrc() {
        return __srcBrowser;
    }
    BrowserAPI.getSrc = getSrc;
    ...
    BrowserAPI.resetLogData = resetLogData;
    ...
    BrowserAPI.enableLogging = enableLogging;
    ...
    BrowserAPI.disableLogging = disableLogging;
    var testData = {
    ...
    BrowserAPI.getTestData = getTestData;
    BrowserAPI.polyfill = !__srcBrowser ? {} : {
        ...
        contextMenus: __srcBrowser.contextMenus ? {
            create: function (createProperties, callback) {
                var id = __srcBrowser.contextMenus.create(createProperties, function () {
                    ...
                    if (__srcBrowser.runtime.lastError) {
                        BrowserAPI.polyfill.runtime.lastError = __srcBrowser.runtime.lastError.message;
                        callback();
                        BrowserAPI.polyfill.runtime.lastError = null;
                    }
                    else {
                        callback();
                    }
            ...
            remove: function (id) {
                if (loggingEnabled) {
                    findItemWithId(testData._currentContextMenu, id, function (item, index, parent) {
            ...
            openOptionsPage: function () {
                return createPromise(function (handler) {
                    if (BrowserAPI.getBrowser() === 'edge') {
                        BrowserAPI.polyfill.tabs.create({
                            url: BrowserAPI.polyfill.runtime.getURL('html/options.html')
                        }).then(function () {
                            handler();
                        });
                    }
...
if (!window.browserAPI || window.__isVirtual) {
    window.browserAPI = (BrowserAPI.getBrowser() === 'edge' || !window.browser) ? __assign({}, BrowserAPI.polyfill, {
        __isProxied: true
    }) :
        window.browserAPI;
